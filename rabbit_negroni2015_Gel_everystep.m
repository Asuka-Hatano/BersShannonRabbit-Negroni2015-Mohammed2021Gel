...
X_w=1.03;
X_p=1.03;
global Lneg force tarray istep Qkarray udotarray;
dt_ms = 1;
...
force=force(1:istep); Lneg=Lneg(1:istep); tarray=tarray(1:istep);
figure(3);
plot(tarray,force);
...
subplot(4,1,4); plot(t,y(:,36),t,y(:,37),t,y(:,31)); title('Ca concs');legend("Caj", "Casl", "Ca_sr");
%subplot(5,1,5); plot(t,y(:,42),t,y(:,43),t,y(:,44)); title('Negroni');legend("T*","TCa","TCa*");
%subplot(5,1,5); plot(tarray,force); title('Negroni force');
figure(2);
subplot(3,1,1);plot(t,y(:,41),t,y(:,43),t,y(:,43));legend("T*","TCa~","TCa*");
subplot(3,1,2);plot(t,y(:,45),t,y(:,46),tarray,Lneg);legend("X_w","X_p","Lneg");
subplot(3,1,3);plot(tarray,force); title('Negroni force');
figure(3);
subplot(2,2,1); plot(t,y(:,42));title("TCa");
subplot(2,2,2); plot(t,y(:,44));title("TCa*");
subplot(2,2,3); plot(t,y(:,43));title("TCa~");
subplot(2,2,4); plot(t,y(:,41));title("T*");
...
global Lneg force istep tarray Qkarray udotarray;
...
if(t==0)
    ...
    Qk=Qkarray(istep-1,:).';
    L=Lneg(istep-1);
    udot0=udotarray(istep-1);
    [L,Fb,Qk,udot1] = IsometHyperGel_variableDt(TCatilde,TCastar,Tstar,X_w,X_p, dt_s,Qk,L,udot0);
    Lneg(istep)=L;
